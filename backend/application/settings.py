...
# A deterministic development fallback keeps a fresh checkout runnable, but
# production deployments must provide DJANGO_SECRET_KEY at runtime.
SECRET_KEY = os.environ.get("DJANGO_SECRET_KEY")
if not SECRET_KEY:
    if DEBUG:
        SECRET_KEY = "dev-only-change-this-django-secret-key"
    else:
...
INSTALLED_APPS = [
    ...
    "channels",
    "dvadmin.system",
    "application.shield",  # 盾构项目管理应用
    "application.ai_assistant",  # AI智能助手
]
...
if DATABASE_ENGINE.startswith("django.db.backends.postgresql"):
...
CELERY_BROKER_URL = locals().get("CELERY_BROKER_URL", REDIS_URL)
CELERY_RESULT_BACKEND = locals().get("CELERY_RESULT_BACKEND", CELERY_BROKER_URL)
# 静态页面压缩
